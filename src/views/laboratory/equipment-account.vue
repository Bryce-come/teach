    ...
    <kit-dialog-simple
      ...
      width="700px">
        ...
        <el-form v-if="modal.deviceInfo" ref="form" :model="modal.deviceInfo" label-width="120px" label-position="left" style="width: 580px;margin: 0 auto">
          <el-form-item label="设备型号" prop="type.name" :rules="{ required: true, message: '请选择设备型号'}">
              <el-select v-model="modal.deviceInfo.type.name"></el-select>
          </el-form-item>
          <el-form-item label="存放地址" prop="extend.address" :rules="{ required: true, message: '请输入存放地址'}">
...
export default {
  setup() {
    ...
    async function update() {
      const valid = true;
      if (valid) {
        const { id } = modal.value.deviceInfo;
    ...
    const query = async () => {
          ...
          producerContact: '', producerTel: '18977538970'}},
          {id: '004', collector: '', name: '切削机', type: {id: '3', name: 'NLJT4', img: '', extend: {ctrName: ''}},
          createDt: '', extend: {ip: '192.168.0.104', buyDt: '2019-12-23', keeper: '王小明', maintenTime: '', address: '浙江自动化学院实验室04', price: '', producer: '浙江金华机床厂',
          producerContact: '', producerTel: '18977538970'}},
          {id: '005', collector: '', name: '切削机', type: {id: '4', name: 'NLJT5', img: '', extend: {ctrName: ''}},
    ...
    return{
        deviceaccountList, buyDate, form, modal, showForm, loading,
