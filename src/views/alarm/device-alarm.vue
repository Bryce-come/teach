  ...
  <div v-loading="loading">
    ...
    <kit-dialog-simple
      ...
      <div>
        ...
        <v-chart
          v-for="(item,index) in paramCharts" :key="'param-chart-'+index+new Date().getTime()" :options="item"
...
import {IParamConfig} from '@/types/beans';
    ...
    const snapshot = async (row: any) => {
      ...
      for (const param of params) {
        ...
        for (const p of list) {
          ...
          if (param.benchmarkMax && p.val) {
            tdataMax.push([p.time, param.benchmarkMax]);
          }
          if (param.benchmarkMin && p.val) {
            tdataMin.push([p.time, param.benchmarkMin]);
          }
          if (min === undefined || p.val < min) {
            min = p.val;
          }
          if (max === undefined || p.val > max) {
            max = p.val;
          }
        }
        ...
        if (min && param.benchmarkMin && param.benchmarkMin < min) {
          min = param.benchmarkMin;
        }
        if (max && param.benchmarkMax && param.benchmarkMax > max) {
          max = param.benchmarkMax;
        }
        if (max) {
